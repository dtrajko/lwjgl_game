...
public class GamepadManager {
	...
	public static void handleInput(Player player, Camera camera) {
		...
		if (gamePadEnabled) {
			if (gamepad.getAxisValue(4) < -0.2f) {
				player.setCurrentSpeed(player.getRunSpeed() * -gamepad.getAxisValue(4));
			}
			if (gamepad.getAxisValue(4) > 0.2f) {
				player.setCurrentSpeed(-player.getRunSpeed() * gamepad.getAxisValue(4));
			}
			if (gamepad.getAxisValue(1) < -0.2f) {
				player.setCurrentTurnSpeed(player.getTurnSpeed());
			}
			if (gamepad.getAxisValue(1) > 0.2f) {
				player.setCurrentTurnSpeed(-player.getTurnSpeed());
			}
			if (gamepad.getAxisValue(2) < -0.2f) {
				float pitch = camera.getPitch();
				camera.setPitch(pitch -= 0.2f);
			}
			if (gamepad.getAxisValue(2) > 0.2f) {
				float pitch = camera.getPitch();
				camera.setPitch(pitch += 0.2f);
			}
			if (gamepad.getAxisValue(3) < -0.2f) {
